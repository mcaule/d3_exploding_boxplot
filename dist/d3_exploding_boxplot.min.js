(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('d3'), require('d3-tip')) :
	typeof define === 'function' && define.amd ? define(['d3', 'd3-tip'], factory) :
	(global.d3_exploding_boxplot = factory(global.d3,global.d3tip));
}(this, (function (d3,d3tip) { 'use strict';

d3tip = d3tip && d3tip.hasOwnProperty('default') ? d3tip['default'] : d3tip;
...
var d3_exploding_boxplot = function (data, aes) {
  // defaults
  var iqr = 1.5;
  var height = 480;
  var width = 600;
  ...
  var colorscale = d3.scale.ordinal().domain(d3.set(data.map(functorkey(aes.color))).values()).range(default_colors);
  ...
  var svg$$1, container, tip;
  var chart = function chart(elem) {
    svg$$1 = d3.select(elem).append('svg').attr('width', width).attr('height', height);

    svg$$1.append('g').append('rect').attr('width', width).attr('height', height).style('color', 'white').style('opacity', 0).on('dblclick', implode_boxplot);

    container = svg$$1.append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
